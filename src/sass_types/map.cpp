...
namespace SassTypes
{
  ...
  Sass_Value* Map::construct(const int len, const std::vector<Local<v8::Value>> raw_val) {
    size_t length = 0;
  ...
  NAN_METHOD(Map::GetValue) {
    ...
    NanNew(Factory::create(sass_map_get_value(map, args[0]->ToInt32()->Value()))->get_js_object());
  }
