...
void CallbackBridge<T, L>::dispatched_async_uv_callback(uv_async_t *req) {
  ...
  argv_v8.push_back(bridge->wrapper);
...
Handle<Function> CallbackBridge<T, L>::get_wrapper_constructor() {
  if (wrapper_constructor.IsEmpty()) {
    ...
    tpl->SetClassName(String::NewSymbol("CallbackBridge"));
    tpl->InstanceTemplate()->SetInternalFieldCount(1);
    tpl->PrototypeTemplate()->Set(
      String::NewSymbol("success"), 
      FunctionTemplate::New(ReturnCallback)->GetFunction()
    );
    wrapper_constructor = Persistent<Function>::New(tpl->GetFunction());
  }
